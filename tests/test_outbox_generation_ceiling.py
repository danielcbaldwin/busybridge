...
async def _seed_op(db, *, start_generation):
    ...
    cur = await db.execute(
        ...
        "VALUES (?, ?, 'create', 'idem-1', 1, 'cal-google-id', 'in_flight')",
        (user_id, proj_id),
    )
...
async def test_resumed_op_at_ceiling_gives_up_without_burning_more(test_db):
    ...
    await _do_create(db, g, op, "cal-google-id", {"summary": "X"}, now=datetime.now(UTC))
